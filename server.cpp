...
string loadEnvValueOrDefault(const string &filename, const string &key,
                             const string &defaultValue) {
  string value = loadEnvValue(filename, key);
  if (value.empty()) {
    return defaultValue;
  }
  return value;
...
int main() {
  ...
  const string dataFile =
      loadEnvValueOrDefault(".env", "OSM_DATA_FILE", "data/uic-fa25.osm.json");
  ...
  Server svr;
  const string mapboxToken = loadEnvValue(".env", "MAPBOX_TOKEN");
  const string mapboxStyleUrl = loadEnvValue(".env", "MAPBOX_STYLE_URL");
  const string serverHost =
      loadEnvValueOrDefault(".env", "SERVER_HOST", "localhost");
  const string serverPortText =
      loadEnvValueOrDefault(".env", "SERVER_PORT", "1251");
  const int serverPort = stoi(serverPortText);
  ...
  svr.Get("/js/config.js", [&](const Request &req, Response &res) {
    (void)req;
    string body = "window.APP_CONFIG = { MAPBOX_TOKEN: \"" +
                  replaceAll(mapboxToken, "\"", "\\\"") +
                  "\", MAPBOX_STYLE_URL: \"" +
                  replaceAll(mapboxStyleUrl, "\"", "\\\"") + "\" };";
  ...
  cout << endl << "Open your browser to http://" << serverHost << ":"
       << serverPort << " to see the map!" << endl;
  cout << "Hit CTRL-C to stop the server." << endl;
  svr.listen(serverHost, serverPort);
}
